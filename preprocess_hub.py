...
with open("lookups/country_iso3.csv", encoding="utf-8") as fh:
...
GLOBAL_CSV = "data/amrorbit/global.csv"
TRACSS_LONG = "outputs/tracss/tracss_raw_long.csv"
...
def trajectory_countries():
    names = set()
    with open(GLOBAL_CSV, newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            for col in ("Spiral In", "Spiral Out", "Constant", "Other"):
                parts = [p.strip() for p in (row.get(col) or "").split(",") if p.strip()]
                i = 0
                while i < len(parts):
                    if parts[i] == "Korea" and i + 1 < len(parts) and parts[i + 1] == "South":
...
def tracss_iso3():
    seen = set()
    with open(TRACSS_LONG, newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            v = (row.get("iso3") or "").strip()
            if v:
                seen.add(v)
    return seen
...
# coverage of the analysis country set
traj_names = trajectory_countries()
traj, traj_dropped = {}, {}
for nm in traj_names:
    iso3, canon, status = resolve_country(nm)
    if status == "mapped":
        traj[iso3] = canon
    elif status == "drop_by_rule":
        traj_dropped[canon] = "not a WHO Member State"
    else:
        raise AssertionError(f"trajectory country unresolved: {nm!r} -> {status}")
tracss = tracss_iso3()
analysis_set = sorted(set(traj) & tracss)
...
for iso3 in analysis_set:
    ...
    cov_rows.append({
        "iso3": iso3, "trajectory_name": traj[iso3],
...
with open(os.path.join(OUT, "hub_coverage_vs_analysis_set.csv"), "w", newline="", encoding="utf-8") as f:
...
prov = {
    ...
    "analysis_country_set": {
        "definition": "AMROrbit trajectory countries (global.csv) that also appear in TrACSS",
        "n_trajectory_countries_raw": len(traj_names),
        "trajectory_dropped_by_rule": traj_dropped,
        "n_analysis_set": len(analysis_set),
...
print(f"analysis set n={len(analysis_set)}; hosted-investment records for "
